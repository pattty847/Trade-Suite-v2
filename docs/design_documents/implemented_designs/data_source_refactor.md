...
*   **`DataFacade` (`data/data_source.py`):** Orchestrates `CacheStore`, `CandleFetcher`, and `Streamer`. It handles exchange communication and caching logic but delegates the heavy lifting to these helper classes. Raw data is placed onto `TaskManager.data_queue` tagged only with market identifiers (`exchange`, `symbol`, `trade_data`/`orderbook`).
