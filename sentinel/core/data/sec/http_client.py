...
class SecHttpClient:
    ...
    def __init__(self, user_agent: str, rate_limit_sleep: float = 0.1):
        ...
        if self.user_agent:
            logging.debug("SecHttpClient using User-Agent: %s", self.user_agent[:50])
        else:
            logging.warning(
                "SecHttpClient has no User-Agent. Set SEC_API_USER_AGENT env (e.g. 'App Name contact@example.com'). SEC requests may fail."
            )
