...
class Streamer:
    ...
    async def watch_trades_list(
        ...
        for exchange_id in self.exchange_list.keys():
            ...
            logging.debug("Starting trade stream for %s on %s", symbols, exchange_id)
            while not stop_event.is_set():
                try:
                    ...
                    logging.debug("Trade list stream for %s on %s cancelled.", symbols, exchange_id)
                    break
                except Exception as e:
                    logging.error(e)
            logging.debug("Trade list stream for %s on %s stopped.", symbols, exchange_id)
    ...
    async def watch_trades(
        ...
        logging.debug("Starting trade stream for %s on %s", symbol, exchange)
        stop_event.clear()
        ...
        while not stop_event.is_set():
            try:
                ...
                logging.debug("Trade stream for %s on %s cancelled.", symbol, exchange)
                break
                ...
                await asyncio.sleep(5)
        logging.debug("Trade stream for %s on %s stopped.", symbol, exchange)

    async def watch_orderbooks(self, symbols: List[str], stop_event: asyncio.Event) -> None:
        for exchange_id in self.exchange_list.keys():
            exchange_object = self.exchange_list[exchange_id]
            logging.debug("Starting orderbook stream for %s on %s", symbols, exchange_id)
            if exchange_object.has["watchOrderBookForSymbols"]:
                while not stop_event.is_set():
                    try:
                        ...
                        logging.debug("Orderbook list stream for %s on %s cancelled.", symbols, exchange_id)
                        break
                    except Exception as e:
                        logging.error(e)
                if not stop_event.is_set():
                    logging.debug("Orderbook list stream stopping for %s due to event clear.", exchange_id)
                    break
            logging.debug("Orderbook list stream for %s on %s stopped.", symbols, exchange_id)
    ...
    async def watch_orderbook(
        ...
        exchange_object = self.exchange_list[exchange]
        logging.debug("Starting orderbook stream for %s on %s with %sms cadence.", symbol, exchange, cadence_ms)
        stop_event.clear()
        ...
        while not stop_event.is_set():
            try:
                ...
                logging.debug("Orderbook stream for %s on %s cancelled.", symbol, exchange)
                break
            except ccxt.NetworkError as e:
                logging.warning(
        ...
        logging.debug("Orderbook stream for %s on %s stopped.", symbol, exchange)
    ...
    async def watch_ticker(self, exchange_id: str, symbol: str, stop_event: asyncio.Event) -> None:
        ...
        logging.debug("Starting ticker watch for %s on %s", symbol, exchange_id)
        stop_event.clear()
        ...
        while not stop_event.is_set():
            try:
                ...
                logging.debug("Ticker watch for %s on %s cancelled.", symbol, exchange_id)
                break
            except ccxt.NetworkError as e:
                await asyncio.sleep(
                ...
                await asyncio.sleep(5)
        logging.debug("Ticker watch for %s on %s stopped.", symbol, exchange_id)
