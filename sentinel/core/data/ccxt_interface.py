...
class CCXTInterface:
    ...
    async def load_exchanges(self, exchanges: List[str] = None):
        ...
        if successful_loads:
            logging.info("Exchanges loaded: %s", ", ".join(successful_loads))
        if self.failed_exchanges:
            logging.warning("Failed to load: %s", self.failed_exchanges)
    ...
    async def close_all_exchanges(self):
        ...
        logging.info("Closing %d exchange(s)...", len(self.exchange_list))
        tasks = []
    ...
    def _get_credentials(self, exchange_id: str) -> Dict[str, str]:
        ...
        if self.force_public:
            logging.debug(f"force_public is True for {exchange_id}, initializing without authentication credentials.")
            return {}
