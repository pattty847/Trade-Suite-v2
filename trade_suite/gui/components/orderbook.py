...
class OrderBook:
    def __init__(self, exchange, emitter: SignalEmitter, data: Data, config: ConfigManager):
    ...
    def toggle_orderbook(self):
        ...
        if self.show_orderbook:
            dpg.configure_item(f"{self.exchange}_order_book_group", show=self.show_orderbook)
            dpg.configure_item(f"{self.exchange}_charts_group", width=dpg.get_viewport_width() * 0.7)
        else:
