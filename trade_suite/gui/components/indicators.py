...
class Indicators:
    ...
    def __init__(self, emitter: SignalEmitter, exchange_settings) -> None:
        ...
        self.indicators = {
            'ema': {'length': {'default': [10, 25, 50, 100, 200], 'source': 'close', 'type': 'int'}},
            'sma': {'length': {'default': [10, 25, 50, 100, 200], 'source': 'close', 'type': 'int'}},
        }
            ...
            for category, indicators in indicators.items():
                with dpg.menu(label=category.capitalize()):
                    for indicator in indicators:
                        dpg.add_checkbox(label=indicator, callback=lambda s, a, u: print(s, a, indicator))
