...
class TA:
    ...
    def calculate_indicators(
        self, exchange_id: str, symbol: str, timeframe: str, df: dict, csv: bool
    ):
        ...
        # Quote volume
        df[quote_volume_title] = (df[base_volume_title] * df["closes"]).apply(
        ...
        df.insert(
            df.columns.get_loc("LOG_RETURNS") + 1,
            "CUM_LOG_RETURNS",
            df["CUM_LOG_RETURN"],
        )
        ...
        df["HULL-55"] = self.hull_moving_average(
            df, 55, insert_next_to="CUM_LOG_RETURN"
        )
        ...
        df.insert(
            df.columns.get_loc("HULL_180") + 1,
            "HULL_55_TREND",
            np.where(df["HULL_55"] >= df["HULL_55"].shift(1), "Bull", "Bear"),
        )
        df.insert(
            df.columns.get_loc("HULL_55_TREND") + 1,
            "HULL_180_TREND",
            np.where(df["HULL_180"] >= df["HULL_180"].shift(1), "Bull", "Bear"),
        )
        ...
        df["SMA_Short_Term_Trend"] = df["SMA_Short_Term_Trend"].apply(
            lambda x: "Bull" if x else "Bear"
        )
        df["SMA_Long_Term_Trend"] = df["SMA_Long_Term_Trend"].apply(
            lambda x: "Bull" if x else "Bear"
        )

        df["SMA_Trend_Strength"] = abs(df["SMA_20"] - df["SMA_50"])
        df["SMA_Trend_Strength"] = pd.cut(
            df["SMA_Trend_Strength"],
            bins=5,
        ...
        df["EMA_Short_Term_Trend"] = df["EMA_Short_Term_Trend"].apply(
            lambda x: "Bull" if x else "Bear"
        )
        df["EMA_Long_Term_Trend"] = df["EMA_Long_Term_Trend"].apply(
            lambda x: "Bull" if x else "Bear"
        )

        df["EMA_Trend_Strength"] = abs(df["EMA_20"] - df["EMA_50"])
        df["EMA_Trend_Strength"] = pd.cut(
            df["EMA_Trend_Strength"],
            bins=5,
            labels=["Neutral", "Weak", "Moderate", "Strong", "Very Strong"],
        )
    ...
    def hull_moving_average(self, data, period, insert_next_to, column="closes"):
        ...
        half_period_wma = self.weighted_moving_average(
        ...
        hma = self.weighted_moving_average(
